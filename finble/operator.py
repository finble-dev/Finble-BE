...
def start():
    ...
    scheduler.add_job(update_kr_data_to_db_daily, CronTrigger.from_crontab('30 16 * * MON-FRI'))
    scheduler.add_job(update_us_data_to_db_daily, CronTrigger.from_crontab('30 6 * * TUE-SAT'))
    scheduler.add_job(update_kospi_data_to_db_daily, CronTrigger.from_crontab('30 16 * * MON-FRI'))
    scheduler.add_job(update_exchangerate_data_to_db_daily, CronTrigger.from_crontab('30 16 * * MON-FRI'))
    scheduler.start()
